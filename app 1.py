...
file_ids = {
    "disease_info.csv": "1jnJmaA49u0TgxMNMiVnDuegjqpr4j65N",   # استبدل بـ ID الصحيح للملف الأول
    "supplement_info.csv": "1NYcWXUNvQsAqh3FbGDUV-Zaz44xdhoGH" # استبدل بـ ID الصحيح للملف الثاني
}
...
# قراءة البيانات بعد التأكد من تحميلها
info = pd.read_csv("disease_info.csv", encoding="latin1")
sup = pd.read_csv("supplement_info.csv")

# تصفية البيانات وإزالة الأمراض غير المرغوبة
info = info[info["disease_name"] != "Raspberry : Healthy"].reset_index(drop=True)
sup = sup[sup["disease_name"] != "Raspberry___healthy"].reset_index(drop=True)
